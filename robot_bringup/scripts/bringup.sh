...
#ros_config="/home/aarnal/workspaces/darko_ws/src/rbkairos_darko_packages/robot_bringup/scripts/ros_config.sh"
source $ros_config
...
if $ROBOT_RUN_SAFETY
then
  echo "Launching safety..."
  screen -S safety -d -m rosrun rosmon rosmon --name=rosmon_safety $ROBOT_BRINGUP_PACKAGE safety_complete.launch;
fi

echo "Launching software in Zed Box..."
if sshpass -p admin ssh user@192.168.0.201 ./bringup.sh
then
  echo "Screens started in Zed Box...";
else
  echo "Error communicating with Zed Box"
fi

sleep 30;

echo "Launching software in CPU Nuvo..."

if sshpass -p R0b0tn1K ssh nuvo@192.168.0.202 ./bringup.sh
then
  echo "Screens started in CPU Nuvo...";
else
  echo "Error communicating with CPU Nuvo"
fi
